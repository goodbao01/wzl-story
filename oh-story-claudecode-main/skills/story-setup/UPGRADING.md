...
- `agents_version: 14` → 旧版，需重新部署以获取 AI 句式硬门槛等 v14 改进
...
- **新增 3 个 Agent**：style-distiller（风格蒸馏）、outline-auditor（大纲偏差审计）、emotion-auditor（人情味审计），Agent 总数从 7 增加到 10。
- **新增 3 个技能路由**：`/story-style-distill`、`/story-audit`。
- **新增角色体系系统**：设定/角色体系.md（9 类角色谱系 + 张力关系表 + 对话风格速查），integration 到写作上下文加载、Phase 3 自动补全、emotion-auditor H2 检定参照。
- **新增模型/平台适配器**：model-adapters/{gpt|claude|deepseek}.md 和 platform-adapters/{fanqie|qidian|zhihu}.md，部署到项目根 `references/`，写作时自动加载对应适配指令。
- **新增双维度审计**：story-long-write Phase 5 集成大纲审计 + 人情味审计。
- **已有项目必须重新运行 `/story-setup` 部署**，部署后须新开会话。
