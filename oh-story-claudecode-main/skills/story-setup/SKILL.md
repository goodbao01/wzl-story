...
| `skills/story-setup/references/templates/agents/*.md` | `.claude/agents/*.md` | story-setup managed | replace | 10 agent files exist |
...
| `skills/story-long-write/references/model-adapters/*.md` | `references/model-adapters/*.md` | story-setup managed | replace | `references/model-adapters/` 下存在 `gpt.md` / `claude.md` / `deepseek.md` 至少一个 |
| `skills/story-long-write/references/platform-adapters/*.md` | `references/platform-adapters/*.md` | story-setup managed | replace | `references/platform-adapters/` 下存在 `fanqie.md` / `qidian.md` / `zhihu.md` 至少一个 |
...
| `skills/story-setup/references/opencode/agents/` | `.opencode/agents/` | story-setup managed | replace | 10 agent files exist | target_cli 含 opencode |
...
- 将 `skills/story-long-write/references/model-adapters/` 下所有 `.md` 复制到项目根 `references/model-adapters/`
- 将 `skills/story-long-write/references/platform-adapters/` 下所有 `.md` 复制到项目根 `references/platform-adapters/`
- 校验：`references/model-adapters/` 下至少存在一个适配器文件；`references/platform-adapters/` 下至少存在一个适配器文件
- 缺失不阻塞部署，但记录警告
...
- 如果 `.story-deployed` 已存在但无 `agents_version` 或版本 < 15，提示用户重新运行 story-setup 以更新 hooks/agents/rules/reference bundle（具体变更见 `UPGRADING.md`）
...
3. 验证 agents：
   - 检查 `.claude/agents/` 下的 10 个 agent 定义文件是否存在
...
5. 验证适配器文件：
   - 检查 `references/model-adapters/` 下是否存在适配器文件（缺失不阻塞，记录警告）
   - 检查 `references/platform-adapters/` 下是否存在适配器文件（缺失不阻塞，记录警告）
6. 验证部署标记：
   - 检查 `.story-deployed` 是否存在且包含时间戳、`agents_version: 15`、`setup_skill_version: 1.2.4`、`target_cli`、`resolver_strategy`、`references_dir`
7. 输出安装报告：
   - 列出所有已部署的文件
   - 列出需要注意的事项（如已有配置已合并）
...
7. 验证 opencode 部署（仅当 target_cli 含 opencode 时）：
    - 检查 `.opencode/agents/` 下的 10 个 agent 定义文件是否存在，且 frontmatter 包含 `mode: subagent` 和 `permission` 字段
...
- `.story-deployed` 存在且 `agents_version: 15` → 提示已部署，AskUserQuestion 确认是否重新部署
- `.story-deployed` 存在但 `agents_version` < 15 → 提示需要更新，重新执行 Phase 2 覆盖 agents/hooks/rules/reference bundle，CLAUDE.md 和 settings.local.json 走合并策略
...
| references/templates/agents/ | 10 个 agent 定义模板（新增：style-distiller, outline-auditor, emotion-auditor，用于风格蒸馏 + 双维度审计） |
...
| skills/story-long-write/references/model-adapters/ | 3 个模型适配器（gpt/claude/deepseek），部署到项目根 `references/model-adapters/` |
| skills/story-long-write/references/platform-adapters/ | 3 个平台适配器（fanqie/qidian/zhihu），部署到项目根 `references/platform-adapters/` |
