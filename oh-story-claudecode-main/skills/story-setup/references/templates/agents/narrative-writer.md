...
| `story-setup/references/agent-references/character-archetypes.md` | 写作或审查对话时，检查角色语气是否匹配其预分配的角色类型（热血/谋略/吐槽/冰山等） |
| `{对标书路径}/文风.md`（绝对路径由 prompt 传入） | prompt 含 `文风路径` 时**写作前必读** |
| `{风格蒸馏路径}/情绪手法.md`（绝对路径由 prompt 传入） | prompt 含 `风格蒸馏摘要` 时按需读取 |
| `{作者思维SKILL路径}`（绝对路径由 prompt 传入） | prompt 含 `author_thinking_指令` 时按需读取「表达DNA」段 |
| `{项目根}/references/model-adapters/{gpt|claude|deepseek}.md` | prompt 含 `model_adapter_指令` 时按需读取 |
| `{项目根}/references/platform-adapters/{fanqie|qidian|zhihu}.md` | prompt 含 `platform_adapter_指令` 时按需读取 |
...
1. 取目标句长带——**优先读 `追踪/上下文.md`「文风指纹」的目标句长带**（每章必加载、compaction 后仍在，是长会话里稳定的锚）；缺失再取 `文风.md` 的「句长分布」字段（短句<15字占比 / 中句 / 长句 / 平均句长，`confidence: high`）；都没有时以「原文锚点片段」语感为准。
