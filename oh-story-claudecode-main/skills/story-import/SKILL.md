...
`设定/题材定位.md` **必须包含「对标书清单 + 主对标书」段**。主对标书最多 1 本；副对标 / 参考对标不限制数量，按用户素材和题材相关性追加到列表，不要截断成”一本副书”。可选字段：`作者思维蒸馏`——如果用户用 Nuwa 蒸馏过自己的思维方式，填写作者名使 OH 在写作时加载对应表达DNA。格式：
...
对标书列表:
  ...
  - 书名: {书名 D}
    ...
    相关性: 同题材/弱相关
    用途: {仅按预算召回摘要}
作者思维蒸馏: {作者名}  # 可选。如用 Nuwa 蒸馏过自己，填作者名，story-long-write 写前加载表达DNA参考
```
