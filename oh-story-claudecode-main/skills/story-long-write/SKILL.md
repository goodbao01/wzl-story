...
- **设定/角色体系.md**：全书角色类型分类表（参考 references/character-archetypes.md）。从角色类型谱系中为每个主要角色选定 1 个主类型 + 可选 1 个副类型，明确角色间的张力关系，建立角色对话风格速查表。格式见下方模板
...
| 角色 | 主类型 | 副类型 | 性格关键词(3-5) | 对话特征(3条) | 核心驱动 |
|------|--------|--------|----------------|--------------|----------|
| 林磊 | A热血莽撞 | C吐槽役 | 直率、重情、嘴笨、冲动 | ①短句为主 ②急了就嚷 ③夸人用拳头 | 保护弟弟 |
| 沈墨 | B冷静谋略 | I高深莫测 | 隐忍、算计、寡言 | ①喜欢反问 ②沉默后再答 ③从不解释 | 复仇布局 |

## 张力关系表

| 组合 | 张力类型 | 典型互动模式 |
|------|----------|-------------|
| 林磊(A) + 沈墨(B) | 互补冲突 | 林磊想冲→沈墨按他→林磊急→沈墨给方案 |
| 林磊(A) + 赵小棠(F) | 互相伤害 | 赵小棠嘴硬→林磊当真→赵小棠更气 |

## 对话风格速查

| 角色 | 基准语气 | 常用句式 | 情绪高点表现 | 情绪低点表现 |
|------|----------|----------|-------------|-------------|
| 林磊 | 急促直白 | "凭什么！""让我来" | 音量提高、拍桌子 | 沉默砸墙 |
| 沈墨 | 平缓克制 | "你想过…吗""不一定。" | 反问变多 | 完全沉默 |
| 赵小棠 | 攻击性防御 | "谁稀罕！""关你事？" | 冷笑+讽刺+音量提高 | 用"哦"结尾 |
```
...
- **新增角色自动入角色体系表**：检查 `设定/角色体系.md` 是否存在。如存在且新角色会复用，则在表中追加该角色的类型分配行：
  - 根据角色设定和细纲对话判断其**主类型**（参考 character-archetypes.md 九大类型）
  - 选填 1 个**副类型**（提供角色反差）
  - 填写 3 个性格关键词
  - 填写 3 条对话特征（从细纲对话中提取或按类型默认）
  - 先与已有角色建立至少 1 个**张力关系**加入张力关系表
  - 追加到对话风格速查表
  - `设定/角色体系.md` 不存在则跳过本步骤
...
| 设定/角色体系.md | 全书 | Phase 2 | Phase 3 张力配对、Phase 4 每章写作前（对话风格速查） |
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如果项目已部署 story-explorer agent（优先检查 `.claude/agents/story-explorer.md` 是否存在；不存在时再检查 `.opencode/agents/`），可 spawn `Agent(subagent_type: "story-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章")` 一次获取上下文）：
   ...
   - (4a) `设定/角色体系.md`（如存在）— 角色类型表 + 对话风格速查
   - (4b) `设定/角色/{相关角色}.md` — 本章涉及角色
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - 3.2 **作者思维框架加载**（无条件执行，独立于对标召回）：从 `设定/题材定位.md` 读取 `作者思维蒸馏` 字段。
     - 如配置了作者名，检查 `.claude/skills/{作者名}-perspective/SKILL.md` 是否存在（即 Nuwa 蒸馏作者自己的产物）
     - 存在 → 读取「表达DNA」段（句式偏好、词汇特征、节奏感、幽默方式），提取 3-5 条该作者的核心表达特征；可选读取「心智模型」前 2 个模型
     - 不存在或未配置 → 跳过，不影响写作
     - 输出：`author_thinking_path`（SKILL.md 路径）, `author_thinking_summary`（表达DNA摘要，一句话写作风格定位），用于合并到 narrative-writer prompt
   - 3.3 **模块召回与文风召回（含风格蒸馏）**：
     - ① 本章目标情绪词？② 借鉴哪个参考文件的哪个技法？③ 用在哪些段落？答不出 → 先回读参考再动笔
     - (a0) **风格蒸馏档案加载**（优先于其他召回）：检查项目根 `风格蒸馏/` 目录是否存在。
       - 从 `设定/题材定位.md` 中读取 `对标书列表` 字段，获取所有对标书名
       - 对每本对标书，检查 `风格蒸馏/{对标书名}/风格摘要.md` 是否存在
       - 存在 → 加载 `风格摘要.md`（获取一句话风格定位 + 技法索引），再按本章需要加载对应的 `{叙事思维.md|情绪手法.md|语言DNA.md|节奏模式.md}`
       - **多书融合**：多本对标书都有风格档案时，**全部加载**。按"共同特征优先，差异特征标注"原则融合：所有蒸馏档案都指向的同一个技法 → 高置信度使用；只有一本提到的技法 → 标注来源备选
       - **隔离规则**：只加载与当前项目同类型的风格档案（通过题材标签匹配）
       - 无 `风格蒸馏/` 目录或无匹配对标书 → 跳过本步骤，回退下方现有文风召回机制
       - 输出：`style_distill_paths`, `style_distill_summary`（各档案一句话摘要）
...
7. **正文执行**：如果项目已部署 narrative-writer agent（**必须优先检查 `.claude/agents/` 下的 `narrative-writer.md` 是否存在；不存在时再检查 `.opencode/agents/`**），先加载模型适配器、平台适配器和作者思维框架：
   - **模型适配器加载**：检测当前使用的模型（从 `设定/模型配置.md` 读取或通过环境变量），加载对应 `references/model-adapters/{gpt|claude|deepseek}.md`。不存在则跳过
   - **平台适配器加载**：从 `设定/题材定位.md` 读取 `目标平台` 字段，加载对应 `references/platform-adapters/{fanqie|qidian|zhihu}.md`。不存在则跳过
   - **作者思维框架加载**：如果前面 3.2 步骤输出了 `author_thinking_path` 和 `author_thinking_summary`，将路径和摘要合并到 narrative-writer prompt 的 `author_thinking_指令` 段
   - 将以上所有适配指令合并到 narrative-writer prompt

然后 spawn narrative-writer agent（prompt 模板见 narrative-writer.md 定义）。如 narrative-writer agent 未部署，由主线程直接写作。
...
写完一批章节后，执行大纲审计检查正文是否偏离规划：

1. 检查 outline-auditor agent 是否可用（`.claude/agents/outline-auditor.md` / `.opencode/agents/outline-auditor.md`）
2. 对本次写作的每章执行审计：
   - **Agent 可用** → spawn `Agent(subagent_type: "outline-auditor", prompt: "项目目录：{dir}\nchapter_num：{N}\ncheck_range：batch\nchapter_list：{本次章节列表}")`
   - **Agent 不可用** → 主线程核对照检查（事件偏差 + 核心冲突偏差 + 情绪目标偏差）
3. 审计结果为 REJECT 的章节 → 提示用户偏差位置，给出修改建议
4. 审计结果为 CONCERNS → 标注偏差，询问用户是否需要修复

#### 人情味审计集成（Phase 5 新步骤）

写完一批章节后，执行人情味审计检查"读起来像人写的吗"：

1. 检查 emotion-auditor agent 是否可用（`.claude/agents/emotion-auditor.md` / `.opencode/agents/emotion-auditor.md`）
2. 对本次写作的每章执行审计：
   - **Agent 可用** → spawn `Agent(subagent_type: "emotion-auditor", prompt: "项目目录：{dir}\nchapter_num：{N}\ntarget_emotion：{从细纲读取}\ncharacter_system：{检测 设定/角色体系.md 是否存在，存在则传 "1"，不存在传 "0"}")`
   - **Agent 不可用** → 主线程执行 7 项检查（H1-H7）
3. 审计评级为 STIFF 或 ROBOTIC 的章节 → 建议先运行 `/story-deslop` 再去 AI 味
4. 审计评级为 NEUTRAL → 标注可优化项供用户参考
...
| 写完，审计偏差或人情味 | story-audit | `/story-audit` |
